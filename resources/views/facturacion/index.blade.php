...
@section('content')
    ...
    <a href="{{ route('VistaPDF') }}" class="btn btn-primary mb-3" target="_blank">PDF</a>
        ...
        <tbody>
            @foreach($facturacion as $facturacion)
                <tr>
                    ...
                    <td>{{$facturacion->id_serie}}</td>
                    <td>
                        <form class="eliminar" action="{{ route('facturacion.destroy',$facturacion->id) }}" method="POST">
                            <a href="{{ route('VistaPDF_UNICO', ['id_serie' => $facturacion->id]) }}" target="_blank" class="btn btn-info"">PDF</a>
